...
// NOTE: This file contains functions adapted from apps.workflow/static/js/job/job_buttons/xero_handlers.js
...
function handlePurchaseOrderXeroButtons(method, online_url = null) {
  ...
  if (!deleteButton || !goToXeroButton) {
    console.error(
      "Required Xero PO action buttons (Delete, GoTo) not found in the DOM.",
    );
    // Submit button might legitimately not exist if the PO is already in Xero, so check separately
    if (method === "DELETE" && !submitButton) {
      console.warn(
        "Submit button not found, but this might be expected if the PO was already in Xero.",
      );
    } else if (method === "POST" && !submitButton) {
      console.warn(
        "Submit button not found after POST operation. This might indicate an issue.",
      );
    }
...
export function deleteXeroPurchaseOrder(purchaseOrderId) {
  ...
  if (!purchaseOrderId) {
    renderMessages(
      [{ level: "error", message: "Purchase Order ID is missing." }],
  ...
  if (
    !confirm(
      "Are you sure you want to delete this Purchase Order from Xero? This cannot be undone.",
    )
  ) {
    return;
  }
                ...
                [
                  {
                    level: "warning",
                    message:
                      "Xero session may have expired. Please try refreshing or re-authenticating.",
                  },
                ],
                "purchase-order-messages",
              );
    ...
    .then((data) => {
      if (!data || !data.success) {
        ...
        renderMessages(
          data?.messages || [
            {
              level: "error",
              message: "Failed to delete Purchase Order from Xero.",
            },
          ],
          "purchase-order-messages",
        );
      ...
      renderMessages(
        data.messages || [
          {
            level: "success",
            message: "Purchase Order successfully deleted from Xero.",
          },
        ],
        "purchase-order-messages",
      );
