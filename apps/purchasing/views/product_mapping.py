...
def product_mapping_validation(request):
    ...
    context = {
        ...
        'stats': {
            ...
            'validation_percentage': round((validated_count / total_mappings * 100) if total_mappings > 0 else 0, 1)
        }
    }
...
def validate_mapping(request, mapping_id):
    ...
    try:
        ...
        mapping.mapped_item_code = request.POST.get('mapped_item_code', mapping.mapped_item_code)
        mapping.mapped_description = request.POST.get('mapped_description', mapping.mapped_description)
        mapping.mapped_metal_type = request.POST.get('mapped_metal_type', mapping.mapped_metal_type)
        mapping.mapped_alloy = request.POST.get('mapped_alloy', mapping.mapped_alloy)
        mapping.mapped_specifics = request.POST.get('mapped_specifics', mapping.mapped_specifics)
        mapping.mapped_dimensions = request.POST.get('mapped_dimensions', mapping.mapped_dimensions)
