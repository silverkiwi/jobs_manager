...
class PurchaseOrderPDFGenerator:
    ...
    def add_header_info(self, y_position):
        ...
        self.pdf.setFont("Helvetica", 12)
        order_date = self.purchase_order.order_date.strftime("%d/%m/%Y") if self.purchase_order.order_date else "N/A"
        ...
        if self.purchase_order.reference:
            ...
            self.pdf.setFont("Helvetica", 12)
            self.pdf.drawString(MARGIN + 80, y_position, str(self.purchase_order.reference))
            y_position -= 20
    ...
    def add_line_items_table(self, y_position):
        ...
        for item in line_items:
            line_total = float(item.quantity * item.unit_cost) if item.quantity and item.unit_cost else 0
            total_amount += line_total
            
            table_data.append([
                str(item.description)[:50] + ("..." if len(str(item.description)) > 50 else ""),
        ...
        # Create table
        lines_table = Table(table_data, colWidths=[CONTENT_WIDTH*0.5, CONTENT_WIDTH*0.15, CONTENT_WIDTH*0.15, CONTENT_WIDTH*0.2])
