...
class MistralPriceExtractionProvider:
    ...
    def _parse_dimensions_from_description(self, description: str) -> Dict[str, Optional[str]]:
        ...
        desc = description.replace('\\mathrm{~mm}', 'mm').replace('\\times', 'x').replace('×', 'x')
        ...
        multi_dim_pattern = r'(\d+(?:\.\d+)?)\s*[xX]\s*(\d+(?:\.\d+)?)\s*(?:[xX]\s*(\d+(?:\.\d+)?))?'
    ...
    def _extract_products_from_markdown_tables(self, markdown_text: str) -> List[Dict[str, Any]]:
            ...
            # Detect table rows (lines with | separators)
                ...
                if len(parts) >= 2:
                    ...
                    product = {
                        ...
                        'product_name': f"{current_category} - {description}" if current_category else description
                    }
    ...
    def _parse_ocr_to_price_data(self, client, ocr_response) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
        ...
        try:
            ...
            logger.info(f"Final parsing results: supplier={supplier_name}, customer={customer_name}, {len(items)} items, {len(pages_data)} pages")
