...
class QuotingTool(MCPToolset):
    ...
    def get_pricing_for_material(self, material_type: str, dimensions: str = None) -> str:
        ...
        for product in products:
            price_info = f"${product.variant_price}" if product.variant_price else "Price TBA"
            results.append(
                f"• {product.supplier.name}: {product.product_name}\n"
    ...
    def create_quote_estimate(self, job_id: str, materials: str, labor_hours: float = None) -> str:
        ...
        relevant_products = SupplierProduct.objects.filter(
            reduce(lambda q, keyword: q | Q(product_name__icontains=keyword) | 
        ...
        if labor_hours:
            ...
            quote_info.append(f"Labor estimate: {labor_hours} hours × $85/hr = ${labor_cost:.2f}")
    ...
    def get_supplier_status(self, supplier_name: str = None) -> str:
        ...
        if supplier_name:
            suppliers = Client.objects.filter(name__icontains=supplier_name, is_supplier=True)
        else:
        ...
        for supplier in suppliers[:10]:  # Limit to 10 suppliers
            ...
            price_lists_count = SupplierPriceList.objects.filter(supplier=supplier).count()
            ...
            if recent_scrape:
                ...
                if recent_scrape.products_scraped:
                    status_info.append(f"  Products Scraped: {recent_scrape.products_scraped}")
            else:
    ...
    def compare_suppliers(self, material_query: str) -> str:
        ...
        ).select_related('supplier').order_by('variant_price')
