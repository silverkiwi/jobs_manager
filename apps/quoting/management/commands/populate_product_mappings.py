...
from apps.quoting.models import SupplierProduct, ProductParsingMapping
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(f"Total SupplierProduct records: {total_supplier_products}")
            self.stdout.write(f"Existing ProductParsingMapping records: {existing_mappings}")
            ...
            self.stdout.write(f"Success rate: {(processed_count / total_to_process * 100):.1f}%")
            self.stdout.write(f"Total time: {timezone.now() - start_time}")
            
            # Updated counts
            new_mapping_count = ProductParsingMapping.objects.count()
            self.stdout.write(f"ProductParsingMapping records: {existing_mappings} → {new_mapping_count}")
    ...
    def _get_processed_hashes(self) -> Set[str]:
        ...
        return set(
            ProductParsingMapping.objects.values_list('input_hash', flat=True)
        )
    ...
    def _get_unprocessed_products(self, processed_hashes: Set[str]) -> list:
        ...
        for product in all_products:
            ...
            if checked_count % 1000 == 0:
                self.stdout.write(f"  Checked {checked_count}/{all_products.count()} products...")
