...
from apps.quoting.services.providers.mistral_provider import MistralPriceExtractionProvider
...
class TestOCRFixtures(TestCase):
    ...
    def test_price_parsing_with_fixtures(self, mock_mistral_class):
        ...
        ocr_files = [f for f in os.listdir(self.ocr_dir) if f.endswith('_ocr_results.json')]
        if not ocr_files:
        ...
        for ocr_file in ocr_files:
            with self.subTest(fixture=ocr_file):
                ...
                with patch('os.path.exists', return_value=True), \
                     patch('apps.quoting.services.providers.mistral_provider.encode_pdf', return_value="mock_base64"):
                ...
                if items:
                    first_item = items[0]
                    required_fields = ['description', 'variant_id', 'category', 'product_name']
                    for field in required_fields:
                        self.assertIn(field, first_item, f"Missing field {field} in {ocr_file}")
                
                print(f"✓ Successfully parsed {ocr_file}: {supplier['name']}, {len(items)} items")
