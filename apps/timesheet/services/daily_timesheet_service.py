...
class DailyTimesheetService:
    ...
    def _get_staff_timesheet_data(cls, staff: Staff, target_date: date) -> Dict:
        ...
        try:
            logger.debug(f"Processing staff: {staff.id} ({type(staff.id)}) for date {target_date}")
            ...
            billable_hours = sum(
                Decimal(line.quantity) for line in cost_lines 
            ...
            staff_data = {
                ...
                'billable_percentage': cls._calculate_percentage(billable_hours, total_hours),
                'completion_percentage': cls._calculate_percentage(total_hours, scheduled_hours),
                'job_breakdown': job_breakdown,
                'entry_count': len(cost_lines),
                'alerts': cls._get_staff_alerts(staff, total_hours, scheduled_hours, cost_lines)
            }
    ...
    @classmethod
    def _determine_status(cls, actual_hours: Decimal, scheduled_hours: Decimal, cost_lines) -> str:
    ...
    @classmethod
    def _get_staff_alerts(cls, staff: Staff, actual_hours: Decimal, scheduled_hours: Decimal, cost_lines) -> List[str]:
    ...
    def _calculate_daily_totals(cls, staff_data: List[Dict]) -> Dict:
        ...
        return {
            ...
            'completion_percentage': cls._calculate_percentage(total_actual, total_scheduled),
            'billable_percentage': cls._calculate_percentage(total_billable, total_actual),
