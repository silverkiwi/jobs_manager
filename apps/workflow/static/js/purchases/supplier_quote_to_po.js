document.addEventListener("DOMContentLoaded", function () {
  ...
  function processQuoteFile(formData) {
    ...
    // Submit the form
    console.log(
      "Sending fetch request to /purchasing/api/supplier-quotes/extract/",
    );
          ...
          return response.json().then((data) => {
            ...
            if (data.success) {
              window.location.href =
                "/purchasing/purchase-orders/new/?quote_data=" +
                encodeURIComponent(JSON.stringify(data.data));
            } else {
              console.error("Error in response data:", data.error);
              alert(
                data.error || "An error occurred while processing the quote.",
              );
            }
          });
        } else {
