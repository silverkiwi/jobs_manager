...
def diff_costset(old: CostSet, drafts: List[DraftLine]) -> DiffResult:
    ...
    if not drafts:
        return DiffResult(
            to_add=[],
            to_update=[],
            to_delete=list(old.cost_lines.all())
        )
    ...
    to_delete = [
        old_line for key, old_line in old_lines_map.items()
...
def _apply_existing_lines(old_cost_set: CostSet, new_cost_set: CostSet, diff_result: DiffResult) -> None:
...
def _create_cost_line_from_draft(cost_set: CostSet, draft: DraftLine, 
                                 existing_ext_refs: Optional[Dict] = None) -> CostLine:
    """
