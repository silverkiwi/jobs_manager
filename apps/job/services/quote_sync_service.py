...
def link_quote_sheet(job: Job, template_url: str | None = None) -> QuoteSpreadsheet:
    ...
    try:
        ...
        if not template_url:
            raise RuntimeError("No master quote template URL configured in CompanyDefaults")
        ...
        if not quotes_folder_id:
            raise RuntimeError("gdrive_quotes_folder_id not configured in CompanyDefaults")
        
        # Ensure 'Jobs Manager' folder exists
        jobs_manager_folder_id = _ensure_jobs_manager_folder(quotes_folder_id, company_defaults)
        
        # Create or locate job sub-folder
        job_folder_name = f"{job.job_number} – {job.name}"
        job_folder_id = _create_or_get_job_folder(jobs_manager_folder_id, job_folder_name)
...
def apply_quote(job: Job):
    ...
    try:
        ...
        if result.success:
            ...
            logger.error(f"Failed to apply quote for job {job.job_number}: {result.error_message}")
...
def _ensure_jobs_manager_folder(parent_folder_id: str, company_defaults: CompanyDefaults) -> str:
    """
    ...
    try:
        ...
        if not hasattr(company_defaults, 'jobs_manager_folder_id') or \
           company_defaults.jobs_manager_folder_id != jobs_manager_folder_id:
