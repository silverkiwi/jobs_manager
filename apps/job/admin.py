...
class CostLineAdmin(admin.ModelAdmin):
    list_display = ('cost_set', 'kind', 'desc', 'quantity', 'unit_cost', 'unit_rev', 'total_cost', 'total_rev')
    ...
    fieldsets = (
        (None, {
            'fields': ('cost_set', 'kind', 'desc')
        }),
        ('Costing', {
            'fields': ('quantity', 'unit_cost', 'unit_rev')
        }),
        ('Metadata', {
            'fields': ('ext_refs', 'meta'),
            'classes': ('collapse',)
        })
    )
