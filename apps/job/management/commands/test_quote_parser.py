...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["legacy"]:
            self.stdout.write(
                self.style.WARNING("Using legacy parser (without new validation)")
            )
        ...
        try:
            ...
            if options["legacy"]:
                ...
                draft_lines, validation_report = parse_xlsx(
                    str(file_path), skip_validation=True
                )
            else:
                # Normal mode: use validation (may reject invalid spreadsheets)
                draft_lines, validation_report = parse_xlsx(
                    str(file_path), skip_validation=False
                )
            ...
            # Basic sanity checks
            self.stdout.write(
                self.style.SUCCESS(f"✓ Successfully parsed {len(lines)} lines")
            )
            ...
            if summary["time_lines"] == 0:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"✓ Found {summary['time_lines']} time lines")
                )
            ...
            if summary["material_lines"] == 0:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✓ Found {summary['material_lines']} material lines"
                    )
                )

            # Calculate breakdown of costs and revenues
            material_cost = sum(
                line.total_cost for line in lines if line.kind == "material"
            )
            time_cost = sum(
                line.total_cost for line in lines if line.kind == "time"
            )  # wage cost
            material_revenue = sum(
                line.total_rev for line in lines if line.kind == "material"
            )
            ...
            material_markup = (
                (material_profit / material_cost * 100) if material_cost > 0 else 0
            )
            time_markup = (time_profit / time_cost * 100) if time_cost > 0 else 0
            total_margin = (
                (total_profit / summary["total_cost"] * 100)
                if summary["total_cost"] > 0
                else 0
            )

            self.stdout.write(
                f"    Material profit: ${material_profit:.2f} ({material_markup:,.1f}% markup)"
            )
            self.stdout.write(
                f"    Time profit: ${time_profit:.2f} ({time_markup:,.1f}% markup)"
            )
            self.stdout.write(
                f"    Total profit: ${total_profit:.2f} ({total_margin:,.1f}% margin)"
            )
            ...
            if (
                has_time_lines
                and has_material_lines
                and not issues
                and has_reasonable_data
            ):
                self.stdout.write(
                    self.style.SUCCESS(
