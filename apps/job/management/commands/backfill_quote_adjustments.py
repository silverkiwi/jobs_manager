...
def get_accounting_date_for_job(job):
    ...
    for entry in job.latest_reality_pricing.time_entries.all():
        if entry.date:
            ...
            print(f"WARNING: Job {job.job_number} has time entry {entry.id} with no date")
    ...
    for entry in job.latest_reality_pricing.material_entries.all():
        if entry.accounting_date:
            ...
            print(f"WARNING: Job {job.job_number} has material entry {entry.id} with no accounting_date")
    ...
    for entry in job.latest_reality_pricing.adjustment_entries.filter(is_quote_adjustment=False):
        if entry.accounting_date:
            ...
            print(f"WARNING: Job {job.job_number} has adjustment entry {entry.id} with no accounting_date")
...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        ...
        for job in target_jobs:
            ...
            existing_quote_adjustments = job.latest_reality_pricing.adjustment_entries.filter(
                is_quote_adjustment=True
            )
