...
from apps.job.models import JobPricing, CostSet, CostLine
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        job_pricings = JobPricing.objects.select_related('job').prefetch_related(
            'time_entries__staff',
            'material_entries',
            'adjustment_entries'
        ).all()
        ...
        if HAS_TQDM:
            ...
            self.stdout.write(f"Processing {job_pricings.count()} JobPricing instances...")
                ...
                with transaction.atomic():
                    ...
                    for time_entry in jp.time_entries.all():
                        cost_line = CostLine(
                            cost_set=cost_set,
                            kind='time',
                            desc=time_entry.description or f"Work - {time_entry.staff.get_display_name() if time_entry.staff else 'No staff'}",
                            quantity=time_entry.hours,
                            unit_cost=time_entry.wage_rate * time_entry.wage_rate_multiplier,
                            unit_rev=time_entry.charge_out_rate * time_entry.wage_rate_multiplier,
                            ext_refs={'time_entry_id': str(time_entry.id)},
                            meta={
                                'staff_id': str(time_entry.staff.id) if time_entry.staff else None,
                                'date': time_entry.date.isoformat() if time_entry.date else None,
                                'is_billable': time_entry.is_billable,
                                'wage_rate_multiplier': float(time_entry.wage_rate_multiplier),
                    ...
                    for material_entry in jp.material_entries.all():
                        cost_line = CostLine(
                            cost_set=cost_set,
                            kind='material',
                            desc=material_entry.description or f"Material - {material_entry.item_code}",
