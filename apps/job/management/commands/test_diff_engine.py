...
from apps.job.diff import diff_costset, apply_diff
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--job-id',
            type=int,
            help='Job ID to test with (will create test cost set)'
        )
        parser.add_argument(
            '--verbose',
            action='store_true',
    ...
    def _show_diff_details(self, diff_result):
        ...
        if diff_result.to_update:
            ...
            for i, (old_line, draft) in enumerate(diff_result.to_update, 1):
                ...
                self.stdout.write(f"     OLD: qty={old_line.quantity} cost=${old_line.unit_cost} rev=${old_line.unit_rev}")
                self.stdout.write(f"     NEW: qty={draft.quantity} cost=${draft.unit_cost} rev=${draft.unit_rev}")
