...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            if not job:
                self.stdout.write(
                    self.style.ERROR('No jobs found in database')
                )
                return

            self.stdout.write(f'Creating test quote data for job: {job.name} ({job.id})')
            ...
            with transaction.atomic():
                ...
                if not quote_costset:
                    ...
                    self.stdout.write(f'Using existing quote costset: {quote_costset.id}')
