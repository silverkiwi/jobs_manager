...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['job_id']:
            try:
                ...
                self.stdout.write(f"Using existing job: [Job {job.job_number}] {job.name}")
        ...
        try:
            if options['preview_only']:
                # Preview only
                self.stdout.write(self.style.WARNING("Preview mode - no actual import will be performed"))
    ...
    def _get_or_create_test_job(self) -> Job:
        ...
        self.stdout.write(f"Created test job: [Job {test_job.job_number}] {test_job.name}")
        return test_job
    ...
    def _test_import(self, job: Job, file_path: str, skip_validation: bool):
        ...
        if current_quote:
            self.stdout.write(f"Current quote: Rev {current_quote.rev} (ID: {current_quote.id})")
        else:
        ...
        if result.success:
            ...
            if result.cost_set:
                self.stdout.write(f"  New CostSet: Rev {result.cost_set.rev} (ID: {result.cost_set.id})")
            ...
            if result.diff_result:
                ...
                self.stdout.write(f"    ➕ Added: {len(result.diff_result.to_add)} lines")
                self.stdout.write(f"    🔄 Updated: {len(result.diff_result.to_update)} lines")
                self.stdout.write(f"    ❌ Deleted: {len(result.diff_result.to_delete)} lines")
