...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--job-id',
            type=int,
            help='Job ID to use for testing (optional)'
        )
        parser.add_argument(
            '--sheet-id',
            type=str,
            help='Google Sheets ID to use for populate test'
        )
    ...
    def test_validation(self):
        ...
        test_cases = [
            ...
            ({'item': 'invalid', 'quantity': 2}, True, 'Non-numeric item (should auto-assign)'),
    ...
    def test_parsing(self):
        ...
        for idx, row in df.iterrows():
            self.stdout.write(f'  Row {idx}: item="{row["item"]}", qty={row["quantity"]}, desc="{row["description"]}"')
        ...
        try:
            ...
            self.stdout.write(f'  - Draft lines created: {len(result.get("draft_lines", []))}')
            self.stdout.write(f'  - Validation report: {result.get("validation_report", {})}')
    ...
    def test_populate(self, sheet_id, job_id=None):
        ...
        if job_id:
            try:
                ...
                self.stdout.write(f'📋 Using existing job: {job.job_number} - {job.name}')
            ...
            if not job:
                self.stdout.write('❌ No jobs found. Create a job first or use --job-id')
                return
            self.stdout.write(f'📋 Using first available job: {job.job_number} - {job.name}')
        ...
        try:
            ...
            self.stdout.write('📊 Data should now be ordered by quantity (highest first) with sequential item numbers')
