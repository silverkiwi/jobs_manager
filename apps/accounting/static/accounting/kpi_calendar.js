...
document.addEventListener("DOMContentLoaded", () => {
  try {
    ...
    const controller = new CalendarController(dataService, view, formatter);

    if (!controller.initialize()) {
      alert(
        "Error during calendar initialization. Check console for more details.",
      );
    }
  } catch (error) {
    console.error("Fatal error initializing KPI calendar reports:", error);
    alert(
      "A serious error ocurred when initializing this page: " + error.message,
    );
  }
});
...
class CalendarView {
  ...
  updateCaptions(thresholds) {
    ...
    document.getElementById("captionAmberMax").textContent =
      this.formatter.formatNumber(greenThreshold - 0.01, 1);
  ...
  #showDayDetailsModal(dayData) {
    ...
    const updateElement = (
      id,
      value,
      formatter = (val) => this.formatter.formatCurrency(val),
    ) => {
    ...
    document.getElementById("dayDetailsModalLabel").textContent =
  ...
  #createProfitDonutChart(dayData, totalProfit) {
    ...
    const segments = [
      {
        label: "Labor",
        value: dayData.details.profit_breakdown.labor_profit,
        color: "#3b82f6",
      },
      {
        label: "Material",
        value: dayData.details.profit_breakdown.material_profit,
        color: "#22c55e",
      },
      {
        label: "Adjustments",
        value: dayData.details.profit_breakdown.adjustment_profit,
        color: "#06b6d4",
      },
    ];
  ...
  async fetchCalendarData(year, month) {
    try {
      const response = await fetch(
        `${this.apiBaseUrl}?year=${year}&month=${month}`,
      );
...
class EventFormatter {
  ...
  formatEvents(calendarData) {
    ...
    const firstHoliday = Object.entries(calendarData || {}).find(
      ([_, data]) => data && data.holiday,
    );
    ...
    if (firstHoliday) {
      console.log("Holiday example:", {
        ...
        dayOfWeek: new Date(firstHoliday[0]).toLocaleDateString("en-NZ", {
          weekday: "long",
        }),
      });
    }
