...
def get_company_profit_loss_data():
    ...
    categories = {
        "Income": {"Sales": [], "Other Revenue": []},
        "Cost of Sales": {"Purchases": [], "Wages": []},
    ...
    for month in months:
        ...
        income_sales = (
            Invoice.objects.filter(date__range=(start_date, end_date)).aggregate(
                Sum("total")
            )["total__sum"]
            or 0
        )
        ...
        cost_purchases = (
            Bill.objects.filter(
                category="Purchases", date__range=(start_date, end_date)
