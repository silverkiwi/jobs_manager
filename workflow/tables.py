...
class ClientTable(tables.Table):
    ...
    class Meta:
        model = Client
        template_code='<a href="{% url "update-client" record.pk %}" class="btn btn-sm btn-primary">Edit</a>',
        template_name = "django_tables2/bootstrap4.html"  # You can choose other templates
        fields = ("name", "email", "phone", "address", "is_account_customer", "edit")

    def render_edit(self, value, record):
        edit_url = reverse("update_client", args=[record.pk])  # Note the changed URL name
