...
from workflow.xero.sync import sync_xero_data
from workflow.xero.xero import get_token, api_client, get_authentication_url, exchange_code_for_token, refresh_token
...
def xero_oauth_callback(request: HttpRequest) -> HttpResponse:
    ...
    if "error" in result:
        return render(request, "workflow/xero_auth_error.html", {"error_message": result["error"]})
...
def refresh_xero_data(request):
    try:
        ...
        if not token:
            logger.info("User is not authenticated with Xero, redirecting to authentication")
