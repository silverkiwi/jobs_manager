...
from workflow.models import (
    AdjustmentEntry, Job, JobPricing, MaterialEntry, 
    TimeEntry, JobFile, Staff
)
...
class SerializerFieldSyncTest(APITestCase):
    ...
    def test_field_coverage(self):
        ...
        if job_files_field and isinstance(job_files_field, serializers.DictField):
            ...
            print("------------------------")
            print("JobFile is handled as a dict in job_files, but these fields need attention:")
            for field in jobfile_fields:
                if field['name'] not in ['id', 'job']:  # Skip obvious fields
                    print(f"- {field['name']} ({field['type']})")
                    if field['name'] == 'print_on_jobsheet':
                        print("  Note: Currently only handled in update() method, not declared in fields")
        ...
        for field in job_fields:
            if field['name'] not in serializer_field_names and field['name'] not in ['id']:
        ...
        if missing_fields:
            ...
            for field in missing_fields:
                print(f"- {field['model']}.{field['name']} ({field['type']}) is not handled in serializer")
        
        # If any issues were found, fail the test
        if (isinstance(job_files_field, serializers.DictField) or 
            missing_fields or 
            any(count > 1 for count in field_counts.values())):
