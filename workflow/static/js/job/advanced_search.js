...
export function setupAdvancedSearch() {
    ...
    document.getElementById("backToKanban").addEventListener("click", function () {
    ...
    document.getElementById("advancedSearchButton").addEventListener("reset", function () {
        setTimeout(() => {
            document.getElementById("searchResultsContainer").style.display = "none";
            document.getElementById("advancedSearchContainer").style.display = "none";
            document.getElementById("kanbanContainer").style.display = "block";
...
function loadClientsDropdown() {
    ...
    fetch("/api/clients/all")
        .then(response => {
            if (Environment.isDebugMode()) console.log("Clients API response status:", response.status);
        ...
        .then(({ data, ok }) => {
            ...
            if (Environment.isDebugMode()) console.log("Clients loaded successfully:", data);
        ...
        .catch(error => {
            ...
            if (Environment.isDebugMode()) console.log("Client loading error details:", error);
        });
}
...
function loadStaffDropdown() {
    ...
    .then(response => {
        if (Environment.isDebugMode()) console.log("Staff API response status:", response.status);
        return response.json().then(data => ({ data, ok: response.ok }));
    })
    .then(({data, ok }) => {
        if (!ok || !Array.isArray(data)) {
        ...
        if (Environment.isDebugMode()) console.log("Staff loaded successfully:", data);
    ...
    .catch(error => {
        ...
        if (Environment.isDebugMode()) console.log("Staff loading error details:", error);
    });
}
...
function executeAdvancedSearch() {
    ...
    if (Environment.isDebugMode()) console.log("Query params:", queryParams.toString());
