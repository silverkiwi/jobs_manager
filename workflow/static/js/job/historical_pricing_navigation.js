...
export function initHistoricalNavigation() {
  ...
  if (window.jobPricingHistory && Array.isArray(window.jobPricingHistory)) {
    ...
    console.log(
      `Historical pricing records found: ${localJobPricingHistory.length}`,
    );
  ...
  // Try alternative source if primary not found
    ...
    window.historical_job_pricings_json &&
    Array.isArray(window.historical_job_pricings_json)
  ) {
    ...
    console.log(
      `Historical pricing records found in alternative source: ${localJobPricingHistory.length}`,
    );
  } else {
  ...
  [
    "historicalPricingNav",
    "prevMonthBtn",
    "nextMonthBtn",
    "returnToCurrentBtn",
    "currentHistoricalMonth",
  ].forEach((id) => {
  ...
  if (localJobPricingHistory.length > 0 && jobStatus === "special") {
    ...
    console.warn(
      "Navigation not shown - either no historical data or not a special job",
    );
  }
...
function updateNavigationButtons() {
  ...
  prevBtn.disabled =
    currentHistoricalIndex >= localJobPricingHistory.length - 1;
...
function loadHistoricalData(index) {
  ...
  if (index >= localJobPricingHistory.length) {
    console.error(
      `Error: Historical data index ${index} out of bounds (max: ${localJobPricingHistory.length - 1})`,
    );
    return;
  }
  ...
  sections.forEach((section) => {
    ...
    updateEntriesGrid(
      section,
      "time",
      historicalData[`${section}_time_entries`] || [],
    );
    ...
    updateEntriesGrid(
      section,
      "material",
      historicalData[`${section}_material_entries`] || [],
    );
    ...
    updateEntriesGrid(
      section,
      "adjustment",
      historicalData[`${section}_adjustment_entries`] || [],
    );
  });
    ...
    ["reality", "estimate", "quote"].forEach((section) => {
      ["time", "material", "adjustment"].forEach((type) => {
        ...
        if (window.grids && window.grids[gridId] && window.grids[gridId].api) {
          ...
          console.log(
            `DELAYED CHECK: ${gridId} has ${currentRows.length} rows`,
          );
          ...
          console.log(
            `DELAYED CHECK: Expected ${expectedEntries.length} rows for ${keyToCheck}`,
          );

          if (currentRows.length !== expectedEntries.length) {
            console.warn(
              `GRID MISMATCH AFTER 1 SECOND: ${gridId} has ${currentRows.length} rows but should have ${expectedEntries.length}`,
            );
          }
        }
      });
...
function forceReplaceGridData(historicalData) {
  ...
  const gridMappings = [
    ...
    {
      gridId: "realityMaterialsTable",
      data: historicalData.material_entries || [],
    },
    {
      gridId: "realityAdjustmentsTable",
      data: historicalData.adjustment_entries || [],
    },
  ];
  ...
  gridMappings.forEach((mapping) => {
    ...
    try {
      ...
      console.log(
        `Method 2: Replacing grid ${gridId} data with ${data.length} historical entries`,
      );
      ...
      console.log(
        `Grid ${gridId} updated successfully with ${data.length} entries`,
      );
    } catch (error) {
...
function forceGridDataClear(gridId) {
  ...
  try {
    ...
    if (gridElement && gridElement.classList.contains("ag-theme-alpine")) {
      ...
      const rowContainer = gridElement.querySelector(
...
function updateEntriesGrid(section, entryType, entriesData) {
  console.log(
    `Updating ${section} ${entryType} grid with ${entriesData.length} entries`,
  );
  ...
  console.log(
    `Trying to update primary grid: ${primaryGridId}, fallback: ${fallbackGridId}`,
  );
  ...
  if (!updated) {
    console.log(
      `Primary grid ${primaryGridId} update failed, trying fallback ${fallbackGridId}`,
    );
...
function updateSectionTotals(section, historicalData) {
  ...
  if (
    window.grids &&
    window.grids[revenueGridId] &&
    window.grids[revenueGridId].api &&
    window.grids[costsGridId] &&
    window.grids[costsGridId].api
  ) {
    try {
      ...
      if (revenueRows.length >= 4) {
        ...
        revenueRows[0][section] =
          historicalData[`${section}_time_revenue`] || 0;
        revenueRows[1][section] =
          historicalData[`${section}_material_revenue`] || 0;
        revenueRows[2][section] =
          historicalData[`${section}_adjustment_revenue`] || 0;
        revenueRows[3][section] =
          historicalData[`${section}_total_revenue`] || 0;
      ...
      if (costRows.length >= 4) {
        ...
        costRows[2][section] =
          historicalData[`${section}_adjustment_cost`] || 0;
    ...
    console.log(
      `${section} revenue or cost tables not found - may be normal if tab isn't active`,
    );
  }
}
...
function compareGridContents(gridId, expectedEntries) {
  ...
  if (actualRows.length !== expectedEntries.length) {
    console.warn(
      `MISMATCH: Grid has ${actualRows.length} rows but should have ${expectedEntries.length}`,
    );
  ...
  if (window.grids) {
    ...
    gridIds.forEach((id) => {
      ...
      if (grid && grid.api) {
        ...
        const capabilities = [];
        if (typeof grid.api.setRowData === "function")
          capabilities.push("setRowData");
        if (typeof grid.api.getRowData === "function")
          capabilities.push("getRowData");
        if (typeof grid.api.sizeColumnsToFit === "function")
          capabilities.push("sizeColumnsToFit");
