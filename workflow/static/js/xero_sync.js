...
class XeroSyncProgress {
  ...
  handleSyncEvent(data) {
    ...
    if (
      data.message.includes("No items to sync") ||
      ...
      this.overallProgress =
        (this.processedEntities / this.totalEntities) * 100;
  ...
  updateEntityRow(entity, updates = {}) {
    ...
    if (updates.lastSync) {
      ...
      row.querySelector(".last-sync").textContent = formatLastSync(
        updates.lastSync,
      );
    }
    if (updates.status) {
  ...
  addLogMessage(message) {
    ...
    if (recordMatch && message.entity) {
      this.updateEntityRow(message.entity, {
        recordsUpdated: parseInt(recordMatch[1], 10),
      });
    }
