...
def create_staff_mapping():
    ...
    first_name = random.choice(['John', 'Jane', 'Michael', 'Sarah', 'David', 'Emma', 'James', 'Lisa'])
    last_name = random.choice(['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia', 'Miller', 'Davis'])
    preferred_name = random.choice(['John', 'Jane', 'Mike', 'Sara', 'Dave', 'Em', 'Jim', 'Liz'])
    ...
    return {
        ...
        'PostalAddress2': random.choice(['Auckland', 'Wellington', 'Christchurch', 'Hamilton', 'Tauranga']),
...
def anonymize_staff_data():
    ...
    for staff in staff_data:
        ...
        if 'raw_ims_data' in fields:
            ...
            for field, value in mapping.items():
                if field in ims_data and ims_data[field] is not None and ims_data[field] != "":
